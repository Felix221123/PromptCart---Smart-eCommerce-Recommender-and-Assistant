...
1. Create a `.env.local` and a `.env` file on the root of the project based on `.env.example`
2. Create your database in any POSTGRESQL admin application like pgAdmin and make sure to replace the credentials in your env and env.local file
3. Execute `$ npm run db:migration:generate` to migrate your entities into the database
4. Start the development server running `$ npm run server`
